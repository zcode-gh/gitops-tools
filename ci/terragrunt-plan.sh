...
if [ ! -f ${TERRAGRUNT_RUNALL_OUTPUT_FILENAME} ]; then
    ...
    # Output the results in a format that can be captured by the calling script. The character "#" at the beginning marks the echo output to be used. The character "|" is used as a delimiter.
...
if [[ ${#COMMENT_OUTPUTS} -gt ${COMMENT_OUTPUTS_MAX_LENGTH} ]]; then
    ...
    COMMENT_OUTPUTS="Content is too long for GitHub comments."
    ENCODED_COMMENT_OUTPUTS=$(echo -e "$COMMENT_OUTPUTS" | jq -Rsa . | base64 -w 0)
...
# Search for error messages in the output file
if grep -q "STDERR" "${TERRAGRUNT_RUNALL_OUTPUT_FILENAME}"; then
    ...
    echo "Error found in the output; job failed..."
    exit 1
fi
